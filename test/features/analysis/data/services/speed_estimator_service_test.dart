...
const _highConfImpact = ImpactResult(frame: 40, confidence: ImpactConfidence.high);
const _lowConfImpact = ImpactResult(frame: 40, confidence: ImpactConfidence.low);
...
void main() {
  ...
  group('SpeedEstimatorService', () {
    test('등속 25km/h 시뮬 + high-confidence impact: 측정 정확', () {
      final detections = <BallDetection?>[
        for (var i = 0; i < 60; i++) BallDetection(cx: 0.5, cy: 0.05 + i * 0.01266, bw: 0.02, bh: 0.02, confidence: 0.9),
      ];
      final r = svc.estimate(
        release: const ReleaseResult(frame: 5, confidence: 0.8),
        impact: _highConfImpact,
        detections: detections, homography: h, sampleFps: 30,
      );
    ...
    test('release 없으면 releaseNotFound', () {
      final r = svc.estimate(
        release: ReleaseResult.notFound, impact: _highConfImpact,
        detections: const [], homography: h, sampleFps: 30,
      );
    ...
    test('비행 윈도우 detections null이면 lowConfidence', () {
      final detections = <BallDetection?>[for (var i = 0; i < 30; i++) null];
      final r = svc.estimate(
    ...
    test('비현실적으로 빠른 속도는 outOfRange', () {
      final detections = <BallDetection?>[
        for (var i = 0; i < 60; i++) BallDetection(cx: 0.5, cy: 0.05 + i * 0.0506, bw: 0.02, bh: 0.02, confidence: 0.9),
      ];
      final r = svc.estimate(
        release: const ReleaseResult(frame: 5, confidence: 0.8),
        impact: _highConfImpact,
